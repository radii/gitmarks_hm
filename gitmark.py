...
class gitMarkImporter(object):
    """ Object used to import data from a set of options and params  """
    def __init__(self, options, args):

        modified = [] # track files we need to add - a hack, because it will add files that are already tracked by git
        url = args[0].strip('/')
        
        base_filename = self.generateHash(url)

        if ( os.path.isfile(os.path.join(CONTENT_PATH,base_filename)) and options['lazyAdd'] == True ):
        ...
        modified.append(self.saveContent(base_filename, content))
        tags = ['all']
        #TODO: klugy. convert tags to be a space seperated list (" to encapsulate tags with space" on import. 
        ...
        for tag in tags:
            ...
            if '/' in t:
                t = ''.join(t.split('/'))
            modified.append(self.saveTagData(t, url, title, base_filename))
    ...
    def saveContent(cls, filename, content):
        content_filename = os.path.join(CONTENT_PATH,filename)
